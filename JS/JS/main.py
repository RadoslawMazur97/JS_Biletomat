...
def Refresh_sum(value, coins_counter, sum_string, change):  # function to refresh sum after throw a coin into machine
    ...
    if (sum_float) <= 0:
        ...
        finish(change_to_be_returned_bool,w)

def finish(change_to_be_returned_bool,text):

    if change_to_be_returned_bool == True:
        t= "Wydano reszte: "
    elif change_to_be_returned_bool == False:
        t= "Tylko odliczona kwota, zwrocono monety: \n"
    tmp2 = []
    tmp =' '.join(str(text) for item in text)
    for k in text:
        tmp2.append(str(float(k))+ "0 zl ")
    print(tmp2)
